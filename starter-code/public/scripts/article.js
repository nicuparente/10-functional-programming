...
(function (module) {
  ...
  */
  ...
  Article.numWordsAll = () => {
    return Article.all.map((article) => article.body.split(' ').length).reduce((total, cur) => total + cur);
  };
  ...
  Article.allAuthors = () => {
    return Article.all.map(function (article) {
      ...
      .reduce(function (authorArr, curAuthor) {
        if (authorArr.indexOf(curAuthor) === -1)
          authorArr.push(curAuthor)
        return authorArr;
      }, []);
  };
    ...
    return Article.allAuthors().map(author => {
      ...
      Article.all.map(article => {
        if (author === article.author) {
          counter += article.body.split(' ').length
        }
      })
      return { authorName: author, wordCount: counter }
    })
  };
